...
export default function TagBadge({ tag, onRemove }: TagBadgeProps) {
    ...
    const backgroundColor = tag.pending ? "transparent" : `${tag.color}15`;
    const borderColor = tag.pending ? `${tag.color}30` : `${tag.color}50`;
    ...
    return (
        ...
        >
            <Link 
                ...
                className="inline-flex items-center gap-1 min-w-0 color-inherit text-current hover:underline"
                style={{ color: "inherit" }}
            >
